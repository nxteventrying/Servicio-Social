...
def process_files(data_folder, output_folder, csv_filename):
    ...
    with open(csv_filename, mode='w', newline='') as csv_file:
        ...
        # Loop through each .txt file and process
        for file_path in txt_files:
            try:
                ...
                rr_mean = np.mean(hrv_data)
                rr_std = np.std(hrv_data)
                row.extend([rr_mean, rr_std])

                # Append SD1 and SD2 for each region
                for region in ['Rest', 'Exercise', 'Recovery']:
                    if region in stats:
                        row.extend([stats[region]['SD1'], stats[region]['SD2']])
                    else:
