        ...
        if filepath:
            try:
                ...
                if filepath.endswith(".csv") or filepath.endswith(".txt"):
                    self.data = pd.read_csv(filepath, sep=",", engine='python')
                ...
                if 'Time' not in self.data.columns or 'RtoR' not in self.data.columns:
                    self.data.columns = ['Time', 'RtoR']

                self.data = self.data.apply(pd.to_numeric, errors='coerce').dropna()
