        ...
        if filepath:
            try:
                if filepath.endswith(".csv"):
                    self.data = pd.read_csv(filepath, header=None, names=['x', 'y'])
                elif filepath.endswith(".txt"):
                    self.data = pd.read_csv(filepath, delim_whitespace=True, header=None, names=['x', 'y'])
                else:
                ...
                if self.data.shape[1] == 2:
                    ...
                    messagebox.showerror("Invalid Data", "File must contain exactly two columns.")
