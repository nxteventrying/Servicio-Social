        ...
        if filepath:
            try:
                if filepath.endswith(".csv"):
                    ...
                    self.data = pd.read_csv(filepath, delim_whitespace=True, header=0)
                else:
                    raise ValueError("Unsupported file format")

                # Clean the data
                self.data.columns = ['x', 'y']  # Ensure the columns are named 'x' and 'y'
                self.data = self.data.dropna()  # Drop rows with missing values
                self.data = self.data.apply(pd.to_numeric, errors='coerce')  # Convert to floats, setting invalid values to NaN
                self.data = self.data.dropna()  # Drop any rows with NaN after conversion
