        ...
        if filepath:
            try:
                if filepath.endswith(".csv"):
                    ...
                    self.data = pd.read_csv(filepath, delim_whitespace=True, header=None)
                else:
                ...
                if self.data.shape[1] == 1:
                    ...
                    self.data = self.data[['x', 'y']]  # Ensure x is the first column
                elif self.data.shape[1] == 2:
                    self.data.columns = ['x', 'y']
                else:
                    raise ValueError("File must contain one or two columns.")
